...
<script>
import $ from 'jquery' // 런타임 오류 잡아주는 주석 ↓ ↓ ↓.
import touchSlider from 'jquery.touchslider' // eslint-disable-line no-unused-vars 
export default {
  mounted() { // 인스턴스에 부탁되고 호출되는 상태
    $('#ex1').click(function(){
      alert("안녕하세요.")
      $(this).next('span').text('인사');
    });
    $('#ex2').click(function(){
      $('.test').css({color : 'red', backgroundColor : 'yellow', width : '100px', height : '100px'});
    });
...
.ts-paging-btn { display:inline-block; width:15px; height:15px; padding-top:10px; margin-left:7px; border: 1px solid #333; border-radius:50%; background:#3399CC; overflow: hidden; font-size: 0;}
.ts-paging-btn:first-child { margin-left: 0; }
.ts-paging-active { background-color:#ff0000; }
...
#touchSlider11 ul li:nth-child(4) {background-color: yellowgreen;}
</style>